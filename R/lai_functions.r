...
build_points_stack <- function(area=NULL){
  temp <- build_area_point_grid(area=area, data=owens_lai)
  temp_id <- dplyr::select(temp, x, y, point.id)
...
get_patch_data <- function(data=NULL, patches=NULL){
  for (i in 1:nrow(patches)){
    n.d <- patches$normal.date[i]
    tmp <- dplyr::filter(data, (normal.date > n.d - 14) & (normal.date < n.d + 14))
...
build_xval_prob_stack <- function(value_stack=NULL, year_index=NULL){
  ticker <- 1
  ticks <- raster::ncell(value_stack) * (raster::nlayers(value_stack) - 1)
  pb <- txtProgressBar(min=0, max=ticks, style=3, width=80)
  prob_raster <- raster::stack()
  for (i in year_index){
    year_raster <- value_stack[[1]] * 0
    prob_raster <- raster::stack(prob_raster, year_raster)
  }
  names(prob_raster)[1:raster::nlayers(prob_raster)] <- paste0("y", year_index)
  for (p in 1:raster::ncell(prob_raster)){
    vert <- drill_down(layers=paste0("y", year_index), stackname=value_stack, point=p)
    for (j in 1:nrow(vert)){
      if (is.na(sum(vert$value))){
        vert$prob[j] <- NA
      } else{
        band <- sd(vert$value[-j])
        kernel <- density(vert$value[-j], n=100, bw=band, from=0)
        kernel_df <- data.frame(x=kernel$x, y=kernel$y)
        diff = abs(vert$value[j] - kernel_df$x)
        prb <- kernel_df$y[which(diff==min(diff))]
        vert$prob[j] <- prb
      }
    }
    for (k in 1:raster::nlayers(prob_raster)){
      prob_raster[p][k] <- vert$prob[k]
      setTxtProgressBar(pb, ticker)
      ticker <- ticker + 1
    }
  }
...
build_xval_quantile_df <- function(data=NULL, column_index=NULL){
  quant_df <- data
  pb <- txtProgressBar(min=0, max=nrow(data), style=3, width=80)
  for (i in 1:nrow(data)){
    history <- as.vector(t(data[i, column_index]))
    quantiles <- c()
    for (j in 1:length(history)){
      kernel_ecdf <- sROC::kCDF(history[-j])
      val <- history[j]
